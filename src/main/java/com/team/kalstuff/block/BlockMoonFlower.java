...
public class BlockMoonFlower extends BlockBush implements ITileEntityProvider
{
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	{
		...
		if ((worldIn.getWorldTime() >= 12800 && worldIn.getWorldTime() < 23000) && ((Integer)state.getValue(NIGHT)).intValue() != 1)
		{
			worldIn.setBlockState(pos, state.withProperty(NIGHT, Integer.valueOf(1)));
		}
		else if ((worldIn.getWorldTime() >= 23000 || worldIn.getWorldTime() < 12800) && ((Integer)state.getValue(NIGHT)).intValue() !=0)
		{
