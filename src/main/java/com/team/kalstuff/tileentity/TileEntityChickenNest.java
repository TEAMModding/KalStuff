...
public class TileEntityChickenNest extends TileEntity implements ITickable
{
    ...
    private boolean hasRoomForItem(ItemStack itemstack)
    {
    	return (this.itemStackHandler.getStackInSlot(0).isEmpty()
    			|| (this.itemStackHandler.getStackInSlot(0).getCount()
    					< this.itemStackHandler.getStackInSlot(0).getMaxStackSize()
    					&& itemstack.isItemEqual(this.itemStackHandler.getStackInSlot(0))));
    }
    ...
    private ItemStack add(ItemStack itemstack)
    {
    	...
    	for (i = 0; i < count && this.hasRoomForItem(itemstack); i ++)
    	{
        	...
        	{
        		this.itemStackHandler.setStackInSlot(0, new ItemStack(itemstack.getItem(), this.itemStackHandler.getStackInSlot(0).getCount() + 1));
        		itemstack.shrink(1);
        	}
    	}
    	return itemstack;
	...
	/**
	 * Taken from net.minecraft.inventory.InventoryHelper, the same code
	 * Vanilla containers use for dropping their items randomly
	 * @param worldIn
	 * @param pos
	 */
